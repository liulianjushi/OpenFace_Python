...
cv::Vec6f LandmarkDetector::GetPose(const LandmarkDetector::CLNF& clnf_model, float fx, float fy, float cx, float cy)
{
...
// The format returned is [Tx, Ty, Tz, Eul_x, Eul_y, Eul_z]
cv::Vec6f LandmarkDetector::GetPoseWRTCamera(const LandmarkDetector::CLNF& clnf_model, float fx, float fy, float cx, float cy)
{
	if (!clnf_model.detected_landmarks.empty() && clnf_model.params_global[0] != 0)
	{
...
bool LandmarkDetector::DetectLandmarksInVideo(const cv::Mat &rgb_image, LandmarkDetector::CLNF& clnf_model, LandmarkDetector::FaceModelParameters& params, cv::Mat& grayscale_image)
{
		...
		|| (clnf_model.tracking_initialised && !clnf_model.detection_success && params.reinit_video_every > 0 && clnf_model.failures_in_a_row % params.reinit_video_every == 0))
	{
		...
		if(face_detection_success)
		{
			...
			bool landmark_detection_success = LandmarkDetector::DetectLandmarksInImage(rgb_image, bounding_box, clnf_model, params, grayscale_image);
...
bool LandmarkDetector::DetectLandmarksInVideo(const cv::Mat &rgb_image, const cv::Rect_<double> bounding_box, LandmarkDetector::CLNF& clnf_model, LandmarkDetector::FaceModelParameters& params, cv::Mat &grayscale_image)
{
	...
	return LandmarkDetector::DetectLandmarksInVideo(rgb_image, clnf_model, params, grayscale_image);
...
bool DetectLandmarksInImageMultiHypBasic(const cv::Mat_<uchar> &grayscale_image, vector<cv::Vec3d> rotation_hypotheses, const cv::Rect_<double> bounding_box, LandmarkDetector::CLNF& clnf_model, LandmarkDetector::FaceModelParameters& params)
{
...
bool DetectLandmarksInImageMultiHypEarlyTerm(const cv::Mat_<uchar> &grayscale_image, vector<cv::Vec3d> rotation_hypotheses, const cv::Rect_<double> bounding_box, LandmarkDetector::CLNF& clnf_model, LandmarkDetector::FaceModelParameters& params)
{
	LandmarkDetector::FaceModelParameters old_params(params);
...
bool LandmarkDetector::DetectLandmarksInImage(const cv::Mat &rgb_image, const cv::Rect_<double> bounding_box, LandmarkDetector::CLNF& clnf_model, LandmarkDetector::FaceModelParameters& params, cv::Mat &grayscale_image)
{
...
bool LandmarkDetector::DetectLandmarksInImage(const cv::Mat &rgb_image, LandmarkDetector::CLNF& clnf_model, LandmarkDetector::FaceModelParameters& params, cv::Mat &grayscale_image)
{
	if (grayscale_image.empty())
	{
		Utilities::ConvertToGrayscale_8bit(rgb_image, grayscale_image);
	...
	if(clnf_model.face_detector_HAAR.empty() && params.curr_face_detector == LandmarkDetector::FaceModelParameters::HAAR_DETECTOR)
	{
		clnf_model.face_detector_HAAR.load(params.haar_face_detector_location);
		clnf_model.haar_face_detector_location = params.haar_face_detector_location;
	}
	
	if (clnf_model.face_detector_MTCNN.empty() && params.curr_face_detector == LandmarkDetector::FaceModelParameters::MTCNN_DETECTOR)
	{
		clnf_model.face_detector_MTCNN.Read(params.mtcnn_face_detector_location);
		clnf_model.mtcnn_face_detector_location = params.mtcnn_face_detector_location;
	...
	{
		return LandmarkDetector::DetectLandmarksInImage(rgb_image, bounding_box, clnf_model, params, grayscale_image);
	}
}
cv::Vec6f LandmarkDetector::testcv2() {
	return cv::Vec6f(1,2,3,4,5,6);
}