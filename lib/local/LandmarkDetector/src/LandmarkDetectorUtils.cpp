...
namespace LandmarkDetector
{
	...
	// Computing landmarks (to be drawn later possibly)
	vector<cv::Point2f> CalculateAllLandmarks(const LandmarkDetector::CLNF& clnf_model)
	{
		return LandmarkDetector::CalculateAllLandmarks(clnf_model.detected_landmarks);
	}

	// Computing landmarks (to be drawn later possibly)
	vector<cv::Point2f> CalculateVisibleLandmarks(const LandmarkDetector::CLNF& clnf_model)
	{
		...
		if (clnf_model.detection_success)
		{
			...
			return LandmarkDetector::CalculateVisibleLandmarks(clnf_model.detected_landmarks, clnf_model.patch_experts.visibilities[0][idx]);
		}
		else
		{
	...
	vector<cv::Point2f> CalculateVisibleEyeLandmarks(const LandmarkDetector::CLNF& clnf_model)
	{
		...
		for (size_t i = 0; i < clnf_model.hierarchical_models.size(); ++i)
		{
				...
				clnf_model.hierarchical_model_names[i].compare("right_eye_28") == 0)
			{
				...
				auto lmks = LandmarkDetector::CalculateVisibleLandmarks(clnf_model.hierarchical_models[i]);
				for (auto lmk : lmks)
				{
	...
	vector<cv::Point3f> Calculate3DEyeLandmarks(const LandmarkDetector::CLNF& clnf_model, float fx, float fy, float cx, float cy)
	{
	...
	// Computing eye landmarks (to be drawn later or in different interfaces)
	vector<cv::Point2f> CalculateAllEyeLandmarks(const LandmarkDetector::CLNF& clnf_model)
	{
		...
		for (size_t i = 0; i < clnf_model.hierarchical_models.size(); ++i)
		{
				...
				clnf_model.hierarchical_model_names[i].compare("right_eye_28") == 0)
			{
				...
				auto lmks = LandmarkDetector::CalculateAllLandmarks(clnf_model.hierarchical_models[i]);
				for (auto lmk : lmks)
				{
					to_return.push_back(lmk);
				}
