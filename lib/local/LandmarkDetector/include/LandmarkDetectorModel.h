...
public:
	...
	LandmarkDetector::Patch_experts		patch_experts;
	...
	vector<LandmarkDetector::CLNF>					hierarchical_models;
	vector<string>					hierarchical_model_names;
	vector<vector<pair<int,int>>>	hierarchical_mapping;
	vector<LandmarkDetector::FaceModelParameters>		hierarchical_params;
	...
	LandmarkDetector::DetectionValidator	landmark_validator;
	...
	bool DetectLandmarks(const cv::Mat_<uchar> &image, LandmarkDetector::FaceModelParameters& params);
...
private:
    ...
    bool Fit(const cv::Mat_<float>& intensity_image, const std::vector<int>& window_sizes, const LandmarkDetector::FaceModelParameters& parameters);
